...
interface AuthUser {
  ...
  role: 'user' | 'superadmin'; status: 'pending' | 'approved' | 'rejected';
}
interface AuthContextType { user: AuthUser | null; loading: boolean; signOut: () => Promise<void>; }
...
const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const loadUser = async (uid: string) => {
    ...
    if (data) setUser({ id: data.id, employeeId: data.employee_id, fullName: data.full_name, role: data.role, status: data.status });
    else setUser(null);
  };
